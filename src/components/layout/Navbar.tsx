...
export default function Navbar({ hospital, customPages = [], navigation }: NavbarProps) {
  ...
  const hospitalName = hospital?.name || 'Oxford Hospital';
  const nameWords = hospitalName.trim().split(/\s+/);
  const firstWord = nameWords[0] || 'OXFORD';
  const restWords = nameWords.slice(1).join(' ') || '';
  const tagline = hospital?.tagline || 'Multi-Speciality & Critical Care';
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-2xs transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          {/* Dynamic Header Logo */}
          <Link href="/" className="flex items-center gap-2.5 sm:gap-3 shrink-0 group py-1.5">
            <img
              src={hospital.logoUrl || '/images/logo.png'}
              alt={hospital.name}
              className="h-11 sm:h-13 w-auto object-contain transition-transform duration-200 group-hover:scale-105 drop-shadow-sm"
            />
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                <span className="font-black text-xl sm:text-2xl text-slate-900 tracking-tight leading-none group-hover:text-[#000066] transition-colors uppercase">
                  {firstWord}
                </span>
                {restWords && (
                  <span className="font-bold text-xl sm:text-2xl text-[#000066] tracking-tight leading-none uppercase">
                    {restWords}
                  </span>
                )}
              </div>
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-2">
                  ...
                  <div key={link.id || link.name} className="relative group">
                    <Link
                      ...
                      className={`flex items-center gap-1 px-3 py-2 rounded-lg text-[14.5px] font-semibold tracking-tight whitespace-nowrap transition-all duration-150 ${
                        isActive
                          ? 'text-[#000066] bg-blue-50/90 font-bold'
                          : 'text-slate-700 hover:text-[#000066] hover:bg-slate-50'
                      }`}
                    >
                      <span>{link.name}</span>
                    ...
                    <div className="absolute top-full left-0 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 hidden group-hover:block animate-in fade-in slide-in-from-top-1 duration-150 z-50">
                      {link.subItems?.map((sub) => (
                        <Link
                          key={sub.id || sub.name}
                          href={sub.href}
                          className="flex items-center justify-between px-4 py-2.5 text-xs sm:text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-[#000066] transition group/item"
                        >
                          <span>{sub.name}</span>
                          <ChevronRight className="w-3.5 h-3.5 text-slate-300 group-hover/item:text-[#000066] transition-transform group-hover/item:translate-x-0.5" />
                        </Link>
                      ))}
                    </div>
                  </div>
            ...
            {/* Dynamic Custom Pages Dropdown if admin creates any */}
            {customPages.filter((p) => p.published).length > 0 && (
              <div className="relative group">
                <button className="flex items-center gap-1 px-3 py-2 rounded-lg text-[14.5px] font-semibold text-slate-700 hover:text-[#000066] hover:bg-slate-50 transition whitespace-nowrap cursor-pointer">
                  <span>Pages</span>
                  ...
                  {customPages
                    ...
                    .map((p) => (
                      <Link
                        key={p.id}
                        href={`/page/${p.slug}`}
                        className="block px-4 py-2.5 text-xs sm:text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-[#000066] transition"
                      >
                        {p.title}
                      </Link>
                    ))}
          ...
          <div className="hidden sm:flex items-center gap-3 shrink-0">
            <Link
              href="/appointment"
              className="bg-gradient-to-r from-[#000066] via-blue-900 to-blue-700 hover:from-blue-950 hover:to-blue-800 text-white text-[13.5px] font-bold px-5 py-2.5 rounded-full shadow-md shadow-[#000066]/25 hover:shadow-lg transition-all transform hover:-translate-y-0.5 flex items-center gap-2 whitespace-nowrap"
            >
          ...
          <div className="flex lg:hidden items-center gap-2">
            <Link
              href="/appointment"
              className="bg-[#000066] text-white px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1 shadow-sm sm:hidden whitespace-nowrap"
            >
        ...
        <div className="lg:hidden border-t border-slate-100 bg-white px-4 pt-3 pb-6 space-y-1 shadow-2xl animate-in slide-in-from-top duration-200">
                ...
                <div key={link.id || link.name} className="space-y-1">
                  <div className="flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50">
                    <Link
                      ...
                      className={isActive ? 'text-[#000066] font-extrabold' : 'text-slate-700'}
                    >
                      {link.name}
                    </Link>
                    <button
                  ...
                  {isSubmenuOpen && (
                    <div className="pl-6 space-y-1 border-l-2 border-[#000066] ml-4 py-1">
                      {link.subItems?.map((sub) => (
                        <Link
                          key={sub.id || sub.name}
                          href={sub.href}
                          onClick={() => setIsOpen(false)}
                          className="block px-3 py-2 text-xs font-semibold text-slate-600 hover:text-[#000066]"
                        >
                          {sub.name}
                        </Link>
                      ))}
            ...
            return (
              <Link
                ...
                className={`block px-4 py-2.5 rounded-xl text-sm font-bold transition ${
                  isActive
                    ? 'text-[#000066] bg-blue-50 font-extrabold'
          ...
          {customPages.filter((p) => p.published).length > 0 && (
            <div className="pt-2 border-t border-slate-100">
              <span className="px-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Pages</span>
              {customPages.filter((p) => p.published).map((p) => (
                <Link
                  key={p.id}
                  href={`/page/${p.slug}`}
                  onClick={() => setIsOpen(false)}
                  className="block px-4 py-2 text-xs font-semibold text-slate-600 hover:text-[#000066]"
                >
                  {p.title}
                </Link>
              ))}
          ...
          <div className="pt-4 border-t border-slate-100 space-y-2">
            <Link
              href="/appointment"
              onClick={() => setIsOpen(false)}
              className="w-full flex items-center justify-center gap-2 bg-[#000066] hover:bg-blue-900 text-white py-3 rounded-full font-bold shadow-md text-center text-sm"
            >
            ...
            >
              <PhoneCall className="w-3.5 h-3.5 text-[#000066]" />
